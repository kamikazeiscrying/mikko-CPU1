...
// EX 模块主要对 ID 传入的两个操作数执行具体的运算操作，并将运算结果传递到流水线的下一级。
// 运算器包括加法器、乘除法器和逻辑运算器
// 通过数据选择器对 EX 阶段的结果进行输出
// 根据 MIPS 规定，乘除法结果存放在 HILO 寄存器中，在 EX 阶段会生产对 HILO 的相关读写信号。
// 此外，EX 级还会处理访存产生的控制信号，将相关信息交给 MEM 级。

`include "bus.v"
`include "funct.v"

module EX(
  // 从 HILO 阶段传递过来的 HI 和 LO 寄存器的读数据
